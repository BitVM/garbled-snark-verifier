...
//!
//! This is the "vanilla" version without VSSS or SP1 soldering:
//! - Publishes ALL input labels for ALL finalized instances
//! - Evaluates ALL finalized instances independently
...
fn run_garbler(
    ...
    // Send ALL finalized inputs to the evaluator
    assert!(!fin_inputs.is_empty(), "no finalized inputs prepared");
    info!("Garbler: sending {} finalized inputs", fin_inputs.len());

    g2e_tx
        .send(SetupBroadcast::AllInputs(fin_inputs))
        .expect("send all evaluator input labels")
}
...
fn run_evaluator(
    ...
    info!(
        "Evaluator selected to finalize indices: {:?}",
        finalize_indices
    );
    ...
    // Receive ALL finalized inputs directly
    let SetupBroadcast::AllInputs(input_cases) = g2e_rx.recv().expect("recv all inputs") else {
        panic!("unexpected message; expected all inputs")
    };

    info!("Evaluator: received {} input cases", input_cases.len());

    // Evaluate all finalized instances
    eval.evaluate_from(&out_dir, input_cases)
        .expect("vanilla evaluate")
}

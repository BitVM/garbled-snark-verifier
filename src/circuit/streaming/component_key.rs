...
pub fn hash_param(name: &str, param_name: &str, param_bytes: &[u8]) -> [u8; 16] {
    generate_component_key(name, [(param_name, param_bytes)])
}
...
mod tests {
    ...
    fn test_key_generation_deterministic() {
        let key1 = generate_component_key("test::component", []);
        let key2 = generate_component_key("test::component", []);
        assert_eq!(key1, key2);
    }
    ...
    fn test_key_generation_with_params() {
        ...
        let key1 = generate_component_key("test::component", [("w", &param_bytes[..])]);
        let key2 = generate_component_key("test::component", [("w", &param_bytes[..])]);
        let param_bytes2 = 11usize.to_le_bytes();
        let key3 = generate_component_key("test::component", [("w", &param_bytes2[..])]);
    ...
    fn test_different_names_different_keys() {
        let key1 = generate_component_key("test::component1", [] as [(&str, &[u8]); 0]);
        let key2 = generate_component_key("test::component2", [] as [(&str, &[u8]); 0]);
        assert_ne!(key1, key2);
    }
}
